...
import { calculateUpgradeAmount } from "@/lib/utils";
...
export const Plans = ({ plans }: PlansProps) => {
  ...
  const createSubscription = useMutation(
    trpc.subscriptionRouter.createSubscription.mutationOptions({
      onSuccess: () => {
        toast.success("구독 완료");
  ...
  const changeSubscription = useMutation(
    trpc.subscriptionRouter.changeSubscription.mutationOptions({
      onSuccess: () => {
        toast.success("구독 변경");
        queryClient.invalidateQueries({
          queryKey: trpc.userRouter.getUserInfo.queryKey(),
  ...
  const registerPaymentMethod = useMutation(
    trpc.subscriptionRouter.upsertPaymentMethod.mutationOptions()
  );
  ...
  const handleSubscribe = async (plan: typeof planTable.$inferSelect) => {
    ...
    if (userInfo.subscription) {
      openDialog({
        ...
        onConfirm: () => {
          changeSubscription.mutateAsync({ planId: plan.id });
        },
      });
    } else {
